    ...
    * Attention : le tableau retourné peut contenir des null si les lignes/colonnes sont vides
    ...
    public Case[] getCasesExtremites(int direction) {
        ...
        for (Case c : this.grille) {
            switch (direction) {
                case HAUT:
                    if ((result[c.getX()] == null) || (result[c.getX()].getY() > c.getY()) || (result[c.getX()].getZ() > c.getZ())) { // si on n'avait pas encore de case pour cette rangée ou si on a trouvé un meilleur candidat
                        result[c.getX()] = c;
                    }
                    break;
                case BAS:
